...
def extract_domestic(wb):
    ...
    ref_date = ws.cell(row=3, column=14).value
    rows = []
    for r in range(6, 45):
        ...
        rows.append({
            ...
            'name': name,
            'exp': fmt_date(ws.cell(row=r, column=8).value),
    ...
    return {'updated': str(updated), 'ref_date': fmt_date(ref_date), 'rows': rows}
...
def build_domestic_html(domestic):
    ...
    for cat, items in groups.items():
        ...
        for i, r in enumerate(items):
            ...
            if r['incoming_qty']:
                incoming = f"<div>{esc(r['incoming_qty'])}</div><div class='sub-date'>{esc(r['incoming_date']) if r['incoming_date'] else ''}</div>"
            else:
...
<td class="exp-col">{esc(r['exp']) if r['exp'] else '—'}</td>
...
<td class="dday-col">{dday_html}</td>
    ...
    if incoming_sorted:
        ...
        findings.append(f"<div class='finding-item'><div class='finding-icon icon-warn'>◐</div><div class='finding-text'>입고 예정 <strong>{len(incoming_items)}건</strong> 중 가장 임박: <strong>{esc(nearest['name'])}</strong> — {esc(nearest['incoming_date'])} (D-{int(nearest['dday']) if isinstance(nearest['dday'], (int, float)) else '?'})</div></div>")
    if dday_errors:
...
def assemble(domestic, amazon, domestic_table, channel_table, findings_domestic, amazon_table, findings_amazon):
...
<style>
  * {{ box-sizing: border-box; margin: 0; padding: 0; }}
  ...
  .tab-nav {{ display: flex; gap: 0; background: #fff; margin: 0 -24px; padding: 0 16px; border-bottom: 1px solid #e5e7eb; box-shadow: 0 2px 8px rgba(0,0,0,0.08); position: relative; }}
...
<table>
<thead><tr><th>구분</th><th style="text-align:left;">제품명</th><th>제품코드</th><th>유통기한</th><th>총재고</th><th>가용재고</th><th>불용(대기)</th><th>입고예정</th><th>D-Day</th></tr></thead>
<tbody>
{domestic_table}
</tbody>
</table>
...
def main():
    ...
    html = assemble(domestic, amazon, domestic_table, channel_table, findings_domestic, amazon_table, findings_amazon)
